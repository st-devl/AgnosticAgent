...
# Max backups to keep per file (prevents disk fill)
...
class DiffGenerator:
    ...
    def _enforce_backup_retention(self):
        """Remove old backups exceeding MAX_BACKUPS_PER_FILE per filename prefix."""
        if not os.path.exists(BACKUP_DIR):
            return
        ...
        for f in os.listdir(BACKUP_DIR):
            ...
            parts = f.rsplit('.', 2)  # splits at last two dots
            base = parts[0] if len(parts) >= 3 else f
            groups.setdefault(base, []).append(f)
        
        for base, files in groups.items():
            # Sort by name (timestamp-based, so alphabetical = chronological)
            sorted_files = sorted(files)
            excess = sorted_files[:max(0, len(sorted_files) - MAX_BACKUPS_PER_FILE)]
