...
### 2. Git Hook'ları Kur (Opsiyonel ama Önerilen)

```bash
bash .agent/hooks/install.sh
```

Bu komut, her `git commit` öncesi otomatik olarak şunları çalıştırır:
- 🛡️ Secret scanner (gizli bilgi taraması — **blocker**)
- 📊 Complexity checker (karmaşıklık uyarısı)
- 🔁 Duplicate detector (kopya kod uyarısı)
